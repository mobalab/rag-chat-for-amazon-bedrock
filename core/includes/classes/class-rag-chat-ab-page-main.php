...
/**
 * Class Rag_Chat_Ab_Page_Main
 *
 * This class handles rendering of the main page.
 *
 * @package     RAGCHATAB
 * @subpackage  Classes/Rag_Chat_Ab_Page_Main
...
class Rag_Chat_Ab_Page_Main {
	...
	function handle_query_form_submission() {
		check_admin_referer( 'rag_chat_ab_query_submit', 'rag_chat_ab_nonce' );
		if ( empty( $_POST['rag_chat_ab_question'] ) ) {
			return;
		}
		...
		$response = $client->retrieve_and_generate( sanitize_text_field( wp_unslash( $_POST['rag_chat_ab_question'] ) ) );
